...
<s:form action="addStaffReal" method="POST">
	...
	<s:select list="#session.crmDepartments" onchange="showPost(this)"  name="crmPost.crmDepartment.depId" listKey="depId" listValue="depName" 	headerKey="depName" headerValue="----请--选--择----" label="所属部门"/>
	<s:if test="crmPost.crmDepartment!=null">
	<s:select list="#session.crmPosts" id="post" name="crmPost.postId" listKey="%{postId}" listValue="%{postName}" headerKey="" headerValue="----请--选--择----" label="职务"/>
	</s:if>
		<s:else>
			<s:select list="{}" id="post"  name="crmPost.postId"  headerKey="" headerValue="----请--选--择----" label="职务"/>
		</s:else>
